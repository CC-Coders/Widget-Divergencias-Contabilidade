...
function BuscaDivergencias(Filtros, Permissao) {
	...
	function ValidaFiltros(Divergencia, Filtros) {
		if (Filtros.Obra != "Todos" && Filtros.Obra.toLowerCase() != Divergencia.OBRA.toLowerCase()) {
			return false;
		}

		if (Filtros.Usuario != "Todos" && Filtros.Usuario.toLowerCase() != Divergencia.CODUSUARIO.toLowerCase()) {
			return false;
		}
	...
	function ValidaPermissoes(Divergencia, Permissao){
		if (Permissao.Permissao == "Geral" || Permissao.Permissao == "Visualizacao") {
			...
			if (Divergencia.CODUSUARIO.toLowerCase() == WCMAPI.userCode.toLowerCase()) {
				return true;
			}
			else{
				return false;
...
function BuscaCamposComplementaresCategoriaDivergencia(ID) {
	var ListaCategorias = [
		...
		},
		{
			ID: 43,
			optFields: [
				{ label: "CNPJ Emitido: ", type: "CNPJ" },
				{ label: "CNPJ Correto: ", type: "CNPJ" }
			]
		}
	];
...
async function NotificaDivergencias(Divergencias) {
	...
	for (const Notificacao of Divergencias) {
		...
		html += "<p>Qualquer dúvida entrar em contato com a Contabilidade, ou acesse o <a href='http://fluig.castilho.com.br:1010/portal/p/1/DivergenciasContabilidade' target='_blank'>Painel de Divergências no Fluig</a>.</p>"
		html += "</div>";
...
async function VerificaPermissoesDoUsuario(user) {
	...
	else if (user == "rodrigo.ramos" || user == "thalessa.tomm") {
