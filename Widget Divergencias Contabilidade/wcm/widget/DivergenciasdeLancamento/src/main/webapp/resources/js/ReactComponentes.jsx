...
function FiltroListaDivergencias({ onBuscaDivergencias, Permissao, OverloadFiltros = {} }) {
    ...
    async function CriaListaNoFiltroPorUsuario() {
        if (Permissao.Permissao == "VisualizacaoUsuario") {
            ...
            handleChangeFiltro("FiltroUsuario", WCMAPI.userCode);
        } else {
            ...
            handleChangeFiltro("FiltroUsuario", "Todos");
    ...
    async function handleBuscaDivergencias(Divergencias) {
        ...
        async function AgrupaDivergenciasPorUsuario(Divergencias) {
            ...
            for (const Divergencia of Divergencias) {
                var found = DivergenciasAgrupadasPorUsuario.find((e) => e.CODUSUARIO.toLowerCase() == Divergencia.CODUSUARIO.toLowerCase());
            ...
            try {
                ...
                for (const Grupo of ds) {
                    if (Grupo["colleagueGroupPK.groupId"] == "Comprador") {
                        emails.push("thalessa.tomm@castilho.com.br");
                        usuarioMatriz = true;
                ...
                if (!usuarioMatriz) {
                    emails.push("rodrigo.ramos@castilho.com.br");
                }
                ...
                return ["contabilidade@castilho.com.br", "rodrigo.ramos@castilho.com.br"].join("; ");
            }
        }
    }
...
function ComponenteChartQuantidadePorUsuario({ Divergencias }) {
    ...
    function ExtraiQuantidadePorCategoria() {
        var listCategorias = [];
        for (const Divergencia of Divergencias) {
            var found = listCategorias.find((e) => e.label.toLowerCase() == Divergencia.CODUSUARIO.toLowerCase());
            if (!found) {
                listCategorias.push({
...
function ComponenteChartQuantidadePorDataDeEmissao({ Divergencias }) {
    ...
    function ComplementaMesesSemData(data) {
        ...
        minDate = minDate[0] + "-" + (parseInt(minDate[1]) + 1) + "-01";
        maxDate = maxDate.split("-");
        maxDate = maxDate[0] + "-" + (parseInt(maxDate[1]) + 1) + "-01";
        ...
        while (startDate <= endDate) {
            console.log("while", startDate, endDate)
            let year = startDate.getFullYear();
            let month = String(startDate.getMonth()).padStart(2, "0");
